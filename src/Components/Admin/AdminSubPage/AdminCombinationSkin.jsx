...
const AdminCombinationSkin = () => {
  ...
  const handleAddClick = async () => {
    const productsRef = ref(database, "products_combinationskin");
  ...
  const handleFormSubmit = async (product) => {
    if (editingProduct) {
      const productRef = ref(
        database,
        `products_combinationskin/${product.id}`
      );
      await set(productRef, product);
    } else {
      const productRef = ref(
        database,
        `products_combinationskin/${newProductId}`
      );
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      const productRef = ref(database, `products_combinationskin/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(
            ref(database, `products_combinationskin/${newIndex}`),
            updatedProduct
          );
          newIndex++;
        }
    ...
    >
      ...
      {showForm && (
        <ProductForm
          ...
          newId={newProductId}
          folder="SkinTypes/combination skin "
        />
      )}
...
export default AdminCombinationSkin;
