...
const AdminSlideMakeup = () => {
  ...
  useEffect(() => {
    const slidesRef = ref(database, "slides_makeup");
  ...
  const handleFormSubmit = async (slide) => {
    if (editingSlide) {
      const slideRef = ref(database, `slides_makeup/${slide.id}`);
      await set(slideRef, slide);
    } else {
      const slideRef = ref(database, `slides_makeup/${newSlideId}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this slide?")) {
      const slideRef = ref(database, `slides_makeup/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(
            ref(database, `slides_makeup/${newIndex}`),
            updatedSlide
          );
          newIndex++;
        }
...
export default AdminSlideMakeup;
