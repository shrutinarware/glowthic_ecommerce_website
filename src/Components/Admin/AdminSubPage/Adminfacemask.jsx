...
const Adminfacemask = () => {
  ...
  const handleAddClick = async () => {
    const productsRef = ref(database, "products_facemask");
  ...
  const handleFormSubmit = async (product) => {
    if (editingProduct) {
      const productRef = ref(database, `products_facemask/${product.id}`);
      await set(productRef, product);
    } else {
      const productRef = ref(database, `products_facemask/${newProductId}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      const productRef = ref(database, `products_facemask/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(
            ref(database, `products_facemask/${newIndex}`),
            updatedProduct
          );
          newIndex++;
        }
