...
const AdminEpillator = () => {
  ...
  const handleAddClick = async () => {
    const productsRef = ref(database, "products_epillators");
  ...
  const handleFormSubmit = async (product) => {
    if (editingProduct) {
      const productRef = ref(database, `products_epillators/${product.id}`);
      await set(productRef, product);
    } else {
      const productRef = ref(database, `products_epillators/${newProductId}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      const productRef = ref(database, `products_epillators/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(
            ref(database, `products_epillators/${newIndex}`),
            updatedProduct
          );
          newIndex++;
        }
