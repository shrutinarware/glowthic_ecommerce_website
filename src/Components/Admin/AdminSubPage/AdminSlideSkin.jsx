...
const AdminSlidSkin = () => {
  ...
  const handleFormSubmit = async (slide) => {
    if (editingSlide) {
      const slideRef = ref(database, `slides_skin/${slide.id}`);
      await set(slideRef, slide);
    } else {
      const slideRef = ref(database, `slides_skin/${newSlideId}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this slide?")) {
      const slideRef = ref(database, `slides_skin/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(ref(database, `slides_skin/${newIndex}`), updatedSlide);
          newIndex++;
        }
      }
    }
