...
const AdminStraightner = () => {
  ...
  const handleAddClick = async () => {
    const productsRef = ref(database, "products_straightner");
  ...
  const handleFormSubmit = async (product) => {
    if (editingProduct) {
      const productRef = ref(database, `products_straighnter/${product.id}`);
      await set(productRef, product);
    } else {
      const productRef = ref(database, `products_straighnter/${newProductId}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      const productRef = ref(database, `products_straighnter/${id}`);
      await remove(productRef);

      // Reindex remaining products
      const productsRef = ref(database, "products_straighnter");
      const snapshot = await get(productsRef);
      if (snapshot.exists()) {
        const productsData = snapshot.val();
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(
            ref(database, `products_straighnter/${newIndex}`),
            updatedProduct
          );
          newIndex++;
        }
...
export default AdminStraightner;
