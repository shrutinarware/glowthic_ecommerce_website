...
const AdminBodywash = () => {
  ...
  const handleAddClick = async () => {
    const productsRef = ref(database, "products_bodywash");
  ...
  const handleFormSubmit = async (product) => {
    if (editingProduct) {
      const productRef = ref(database, `products_bodywash/${product.id}`);
      await set(productRef, product);
    } else {
      const productRef = ref(database, `products_bodywash/${newProductId}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      const productRef = ref(database, `products_bodywash/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(
            ref(database, `products_bodywash/${newIndex}`),
            updatedProduct
          );
          newIndex++;
        }
