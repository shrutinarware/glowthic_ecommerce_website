...
const AdminBrands = () => {
  ...
  const handleFormSubmit = async (product) => {
    if (editingProduct) {
      const productRef = ref(database, `brands/${product.id}`);
      await set(productRef, product);
    } else {
      const productRef = ref(database, `brands/${newProductId}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      const productRef = ref(database, `brands/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(ref(database, `brands/${newIndex}`), updatedProduct);
          newIndex++;
        }
      }
    }
      ...
      <div>
        {products
          .filter((item) => {
            ...
            return name.includes(search) || link.includes(search);
          })
