...
const Adminunisexperfume = () => {
  ...
  const handleAddClick = async () => {
    const productsRef = ref(database, "products_unisexperfume");
  ...
  const handleFormSubmit = async (product) => {
    if (editingProduct) {
      const productRef = ref(database, `products_unisexperfume/${product.id}`);
      await set(productRef, product);
    } else {
      const productRef = ref(
        database,
        `products_unisexperfume/${newProductId}`
      );
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      const productRef = ref(database, `products_unisexperfume/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(
            ref(database, `products_unisexperfume/${newIndex}`),
            updatedProduct
          );
          newIndex++;
        }
    ...
    >
      ...
      {showForm && (
        <ProductForm
          ...
          newId={newProductId}
          folder="Fragnance/unisexperfume"
        />
      )}
