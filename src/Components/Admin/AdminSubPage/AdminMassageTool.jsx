...
const AdminMassage = () => {
  ...
  const handleAddClick = async () => {
    const productsRef = ref(database, "products_massagetools");
  ...
  const handleFormSubmit = async (product) => {
    if (editingProduct) {
      const productRef = ref(database, `products_massagetools/${product.id}`);
      await set(productRef, product);
    } else {
      const productRef = ref(database, `products_massagetools/${newProductId}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      const productRef = ref(database, `products_massagetools/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(
            ref(database, `products_massagetools/${newIndex}`),
            updatedProduct
          );
          newIndex++;
        }
    ...
    >
      ...
      {showForm && (
        <ProductForm
          ...
          newId={newProductId}
          folder="Appliances/massage tools"
        />
      )}
