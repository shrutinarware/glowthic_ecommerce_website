...
const AdminDryer = () => {
  ...
  const handleAddClick = async () => {
    const productsRef = ref(database, "products_dryer");
  ...
  const handleFormSubmit = async (product) => {
    if (editingProduct) {
      const productRef = ref(database, `products_dryer/${product.id}`);
      await set(productRef, product);
    } else {
      const productRef = ref(database, `products_dryer/${newProductId}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      const productRef = ref(database, `products_dryer/${id}`);
      ...
      if (snapshot.exists()) {
        ...
        for (const [, value] of sortedEntries) {
          ...
          await update(
            ref(database, `products_dryer/${newIndex}`),
            updatedProduct
          );
          newIndex++;
        }
